...
source.new = function()
    return setmetatable({ signature_help = nil }, { __index = source })
end

source.is_available = function(self) return self:_get_client() ~= nil end

source.get_keyword_pattern = function(self)
    return ([=[\%%(\V%s\m\)\s*\zs]=]):format(table.concat(
...
source.get_trigger_characters = function(self)
    ...
    for _, c in ipairs(self:_get(self:_get_client().server_capabilities,
        { 'signatureHelpProvider', 'triggerCharacters' }) or
        {}) do table.insert(trigger_characters, c) end
    for _, c in ipairs(self:_get(self:_get_client().server_capabilities, {
        'signatureHelpProvider', 'retriggerCharacters'
    }) or {}) do table.insert(trigger_characters, c) end
...
source.complete = function(self, params, callback)
    ...
    local trigger_characters = {}
    for _, c in ipairs(self:_get(client.server_capabilities,
        { 'signatureHelpProvider', 'triggerCharacters' }) or
        {}) do table.insert(trigger_characters, c) end
    for _, c in ipairs(self:_get(client.server_capabilities, {
        'signatureHelpProvider', 'retriggerCharacters'
    ...
    for _, c in ipairs(trigger_characters) do
        local s, e = string.find(params.context.cursor_before_line,
            '(' .. vim.pesc(c) .. ')%s*$')
        if s and e then
            trigger_character = string.sub(params.context.cursor_before_line, s,
                s)
            break
        end
    end
    if not trigger_character then return callback({ isIncomplete = true }) end

    local request = vim.lsp.util.make_position_params(0,
        self:_get_client().offset_encoding)
    request.context = {
        triggerKind = 2,
    ...
    client.request('textDocument/signatureHelp', request,
        function(_, signature_help)
            self.signature_help = signature_help

            if not signature_help then return callback({ isIncomplete = true }) end

            self.signature_help.activeSignature = self.signature_help
                .activeSignature or 0
            callback({ isIncomplete = true, items = self:_items(self.signature_help) })
        end)
end

source._items = function(self, signature_help)
    if not signature_help or not signature_help.signatures then return {} end
...
source._item = function(self, signature, parameter_index)
    ...
    if #parameters < parameter_index then parameter_index = 1 end
    ...
    if #arguments == 0 then return nil end
...
source._docs = function(self, signature, parameter_index)
    ...
    if signature.label then
        table.insert(documentation,
            self:_signature_label(signature, parameter_index))
    end
...
source._signature_label = function(self, signature, parameter_index)
    ...
    if parameter_index then
        local s, e = string.find(label, self:_parameter_label(signature,
            signature.parameters[parameter_index]),
            1, true)
        if s and e then
            local active = string.sub(label, s, e)
            label = string.gsub(label, vim.pesc(active),
                '***' .. active .. '***')
        end
    end
    return label
...
source._parameter_label = function(_, signature, parameter)
    ...
    if type(label) == 'table' then
        label = signature.label:sub(1 +
            vim.str_byteindex(signature.label,
                label[1]),
